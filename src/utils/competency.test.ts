...
describe("buildCompetencySummary", () => {
  it("treats study sheets, teaching decks, cases, and quiz performance as the required week objectives", () => {
    const completedItems = {
      articles: {},
      studySheets: Object.fromEntries((STUDY_SHEETS[1] || []).map((sheet) => [sheet.id, true])),
      decks: Object.fromEntries(CURRICULUM_DECKS.filter((deck) => deck.week === 1).map((deck) => [deck.id, true])),
    ...
    expect(summary.objectives.map((objective) => objective.label)).toEqual(["Study sheets", "Teaching decks", "Cases", "Quiz"]);
