...
function StudentViewRouter({
  ...
  return (
    <>
        {tab === "today" && !subView && <HomeTab navigate={navigate} preScore={preScore} postScore={postScore} curriculum={curriculum} articles={articles} studySheets={studySheets} announcements={announcements} currentWeek={currentWeek} totalWeeks={totalWeeks} rotationEnded={rotationEnded} weeklyScores={weeklyScores} completedItems={completedItems} bookmarks={bookmarks} srDueCount={getDueItems(srQueue).length} patients={patients} online={online} competencySummary={competencySummary} gamification={gamification} reflections={reflections} onSubmitReflection={onSubmitReflection} installPromptVariant={installPromptVariant} onInstallApp={onInstallApp} onDismissInstallPrompt={onDismissInstallPrompt} onCompleteConsultTopic={onCompleteConsultTopic} />}
