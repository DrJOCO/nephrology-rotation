  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <input
        ...
        style={{ width: "100%", padding: "12px 14px", border: `1.5px solid ${T.line}`, borderRadius: 10, fontSize: 14, boxSizing: "border-box", marginBottom: 14, fontFamily: T.sans }}
      ...
      <div style={{ background: T.card, borderRadius: 14, border: `1px solid ${T.line}`, overflow: "hidden" }}>
        ...
        {filtered.map((a, i) => (
          <div key={i} style={{ padding: "10px 16px", borderBottom: i < filtered.length - 1 ? `1px solid ${T.bg}` : "none", display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <div style={{ fontSize: 13, color: T.ink, lineHeight: 1.4, wordBreak: "break-word" }}>{a.full}</div>
          </div>
        ))}
      </div>
