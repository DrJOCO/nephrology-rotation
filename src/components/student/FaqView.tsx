...
export default function FaqView({ onBack }) {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <h2 style={{ color: T.ink, fontSize: 20, margin: "0 0 4px", fontFamily: T.serif, fontWeight: 700 }}>Rotation FAQ</h2>
        ...
        return (
          <div key={i} style={{ background: T.card, borderRadius: 12, marginBottom: 8, border: `1px solid ${isOpen ? T.brand : T.line}`, overflow: "hidden", transition: "border 0.2s" }}>
            <button onClick={() => setOpenFaq(isOpen ? null : i)}
              style={{ width: "100%", padding: "14px 16px", background: isOpen ? T.surface2 : T.card, border: "none", cursor: "pointer", textAlign: "left", display: "flex", alignItems: "flex-start", gap: 10 }}>
              <span style={{ color: T.brand, fontWeight: 700, fontSize: 14, flexShrink: 0, marginTop: 1 }}>Q</span>
              <div style={{ flex: 1, minWidth: 0, fontWeight: 600, color: T.ink, fontSize: 14, lineHeight: 1.4 }}>{faq.q}</div>
      ...
      {faqs.length > 3 && <BackButton onClick={onBack} placement="inline" style={{ marginTop: 20, marginBottom: 0 }} />}
    </div>
  );
}
