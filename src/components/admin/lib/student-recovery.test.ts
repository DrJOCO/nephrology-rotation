...
import { buildRecoveredStudent, performStudentRecovery, type RecoverySyncStore, type StudentWriteResult } from "./student-recovery";
...
function makeFakeStore(writeResult: StudentWriteResult) {
  const fake = {
    setStudentData: vi.fn(async () => writeResult),
